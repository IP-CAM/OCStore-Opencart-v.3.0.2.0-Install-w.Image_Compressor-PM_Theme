...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (substr(str_replace('\\', '/', realpath($directory) . '/' . $filter_name), 0, strlen(DIR_IMAGE . 'catalog')) == str_replace('\\', '/', DIR_IMAGE . 'catalog')) {
			...
			// Get files
			$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF,svg,SVG}', GLOB_BRACE); // ! adding ,svg,SVG (SVG-support)
	...
	public function upload() {
		...
		if (!$json) {
            ...
            'svg', // ! adding (SVG-support)
					...
					$allowed = array(
            'image/jpeg',
            'image/svg+xml',  // ! adding (SVG-support)
						'image/pjpeg',
						'image/png',
						'image/x-png',
						'image/gif'
