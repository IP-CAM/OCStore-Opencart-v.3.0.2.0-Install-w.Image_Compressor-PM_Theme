...
class CustomFieldsCheckbox {
	...
	public function renderFrontendView($result, $language_id, &$controller) {
		...
		$result['data'][$language_id]['value'] = isset($result['data'][$language_id]['value'])?html_entity_decode($result['settings_data'][$language_id]['checked'], ENT_QUOTES, 'UTF-8'):'';
	...
	public function renderFrontendAdvanced($result, $language_id, &$controller) {
		...
		$result['data'][$language_id]['value'] = isset($result['data'][$language_id]['value'])?$result['settings_data'][$language_id]['checked']:'';
	...
	public function makeArray($result, $language_id, &$controller){
		...
		$ret = array(
			'data'	=> isset($result['data'][$language_id]['value'])? html_entity_decode($result['settings_data'][$language_id]['checked'], ENT_QUOTES, 'UTF-8'):'',
		);
		return $ret;
	}
