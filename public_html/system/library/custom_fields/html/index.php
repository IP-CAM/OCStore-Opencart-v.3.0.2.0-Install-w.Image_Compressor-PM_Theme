...
class CustomFieldsHtml {
	...
	public function renderFrontendView($result, $language_id, &$controller) {
		$result['data'] = unserialize($result['data']);
		//require_once('language/'.$language.'.php');
		$result['data'][$language_id]['text'] = html_entity_decode($result['data'][$language_id]['text'], ENT_QUOTES, 'UTF-8');
	...
	public function renderFrontendAdvanced($result, $language_id, &$controller) {
		$result['data'] = unserialize($result['data']);
		return html_entity_decode($result['data'][$language_id]['text'], ENT_QUOTES, 'UTF-8');
	}
	
	public function makeArray($result, $language_id, &$controller){
		$result['data'] = unserialize($result['data']);
		$ret = array(
			'text'	=> html_entity_decode($result['data'][$language_id]['text'], ENT_QUOTES, 'UTF-8'),
		);
		return $ret;
	}
